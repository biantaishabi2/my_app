defmodule MyApp.FormLogicTest do
  ...
  setup do
    ...
    # 添加性别选项
    {:ok, _male_option} = Forms.add_item_option(gender_item, %{
      label: "男",
      value: "male"
    })
    
    {:ok, _female_option} = Forms.add_item_option(gender_item, %{
      label: "女",
      value: "female"
    })
    
    # 创建表单项 - 年龄数字框 - 必填项
    {:ok, age_item} = Forms.add_form_item(form, %{
      label: "年龄",
      type: :number,
      required: true,
      order: 2,
    ...
    # 创建表单项 - 婚姻状况单选框 - 必填项
    {:ok, marital_item} = Forms.add_form_item(form, %{
      label: "婚姻状况",
      type: :radio,
      required: true,
      order: 3
    ...
    # 添加婚姻状况选项
    {:ok, _single_option} = Forms.add_item_option(marital_item, %{
      label: "未婚",
      value: "single"
    })
    
    {:ok, _married_option} = Forms.add_item_option(marital_item, %{
      label: "已婚",
      value: "married"
    })
    
    # 创建表单项 - 子女数量框 - 非必填项
    {:ok, children_item} = Forms.add_form_item(form, %{
      label: "子女数量",
      type: :number,
      required: false,
      order: 4,
  ...
  describe "条件创建测试" do
    ...
    test "嵌套条件", %{gender_item: gender_item, age_item: age_item} do
      ...
      nested_condition = FormLogic.build_compound_condition("or", [and_condition1, and_condition2])
  ...
  describe "条件应用到表单项测试" do
    test "设置可见性条件", %{gender_item: gender_item, age_item: age_item, children_item: children_item} do
      ...
      # 设置子女数量表单项的可见性条件
      {:ok, updated_item} = Forms.add_condition_to_form_item(children_item, nested_condition, :visibility)
    ...
    test "表单项可见性条件评估", %{form: form, gender_item: gender_item, age_item: age_item, children_item: children_item} do
      # 创建婚姻状况 = 已婚条件
      ...
      # 设置子女数量表单项的可见性条件
      {:ok, updated_children_item} = Forms.add_condition_to_form_item(children_item, condition, :visibility)
    ...
    test "表单项必填条件评估", %{form: form, gender_item: gender_item, age_item: age_item, children_item: children_item} do
      # 创建婚姻状况 = 已婚条件
      {:ok, marital_item} = Forms.get_form_item_by_label(form.id, "婚姻状况")
      condition = FormLogic.build_condition(marital_item.id, "equals", "married")
      
      # 设置子女数量表单项的必填条件
      {:ok, updated_children_item} = Forms.add_condition_to_form_item(children_item, condition, :required)
