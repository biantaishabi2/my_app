defmodule MyApp.ResponsesExportTest do
  ...
  defp setup_form_with_items(user_id) do
    ...
    # 添加多选题
    {:ok, checkbox_item} = Forms.add_form_item(form, %{
      label: "多选题",
      type: :checkbox,
      required: false,
      order: 2
    ...
    # 添加评分题
    {:ok, _rating_item} = Forms.add_form_item(form, %{
      label: "评分题",
      type: :rating,
      required: true,
      order: 3,
    ...
    # 添加文本题
    {:ok, _text_item} = Forms.add_form_item(form, %{
      label: "文本题",
      type: :text_input,
      required: false,
      order: 4
  ...
  defp create_test_response(form, submitted_at) do
    ...
    answers = Enum.map(form_items, fn item ->
      case item.type do
        :radio ->
    ...
    # 更新提交时间，确保所有时间字段都有值
    # 确保时间字段类型匹配
    {:ok, updated_response} = Repo.update(Ecto.Changeset.change(response, 
  ...
  defp create_responses_with_choice(form, item_id, option_id, count) do
    Enum.each(1..count, fn _ ->
      ...
      base_answers = Enum.map(form_items, fn item ->
  ...
  describe "export_responses/2" do
    ...
    test "exports responses with correct answer values", %{form: form} do
      ...
      # 验证导出数据包含正确的答案值
      text_answer = Enum.find(response.answers, fn a -> 
        # Get form items from pages to ensure we have access to them
    ...
    test "handles empty response set", %{user: user} do
      # 确保没有回答数据
      {:ok, form_copy} = Forms.create_form(%{
        title: "无回答表单",
  ...
  describe "error handling" do
    test "returns error when form does not exist" do
      result = Responses.export_responses("00000000-0000-0000-0000-000000000999", %{format: "csv"})
    ...
    test "returns error with invalid date format" do
      ...
      {:ok, test_form} = Forms.create_form(%{
        title: "测试日期表单",
      ...
      {:ok, _page} = Forms.create_form_page(test_form, %{
        title: "第一页",
        description: "测试页面",
        order: 1
      })
      ...
      # 使用有效的表单ID但无效的日期格式
      result = Responses.export_responses(test_form.id, %{format: "csv", start_date: "invalid-date"})
