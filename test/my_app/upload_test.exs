defmodule MyApp.UploadTest do
  ...
  describe "uploaded_files" do
    ...
    test "save_uploaded_file/4 with valid data creates a file record", %{
      form: form,
      form_item: form_item
    } do
    ...
    test "save_uploaded_file/4 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} =
               Upload.save_uploaded_file("form_id", "item_id", @invalid_attrs)
    end

    test "get_files_for_form_item/2 returns files for a form item", %{
      form: form,
      form_item: form_item
    } do
      ...
      {:ok, _file2} =
        Upload.save_uploaded_file(form.id, other_form_item.id, %{
          @valid_attrs
          | filename: "xyz789.jpg"
        })
    ...
    test "get_files_for_form/1 returns files grouped by form item", %{
      form: form,
      form_item: form_item
    } do
      ...
      {:ok, file2} =
        Upload.save_uploaded_file(form.id, form_item.id, %{@valid_attrs | filename: "def456.jpg"})
    ...
    test "associate_files_with_response/3 associates files with a response", %{
      form: form,
      form_item: form_item
    } do
      # Create two files
      ...
      {:ok, response} =
        MyApp.Repo.insert(%MyApp.Responses.Response{
          form_id: form.id,
          submitted_at: now,
          respondent_info: %{},
          inserted_at: now,
      ...
      # two files for that item
      assert length(files[form_item.id]) == 2
