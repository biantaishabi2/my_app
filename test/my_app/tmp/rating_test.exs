  ...
  describe "rating component" do
    ...
    test "create a form with rating component", %{user: user} do
      ...
      {:ok, rating_item} = Forms.add_form_item(form, %{
        ...
        "required" => true,
        "order" => 1,
        "max_rating" => "10"  # 设置最大评分为10，模拟表单提交字符串值
      })
      ...
      assert rating_item.max_rating == 10  # 现在应该能正确保存max_rating值了
      assert rating_item.required == true

      # 通过ID获取评分组件并验证
      retrieved_item = Forms.get_form_item(rating_item.id)
      IO.inspect(retrieved_item, label: "获取的评分组件")
      assert retrieved_item.max_rating == 10  # 验证max_rating正确保存到了数据库
      assert retrieved_item.type == :rating

      # 更新评分组件
      {:ok, updated_item} = Forms.update_form_item(rating_item, %{
        "max_rating" => "5",
        "label" => "更新后的评分标签"
      })
      
      IO.inspect(updated_item, label: "更新后的评分组件")
      assert updated_item.max_rating == 5  # 现在应该正确更新为5
