defmodule MyAppWeb.FormLive.SubmitPagedTest do
  ...
  describe "分页表单提交" do
    ...
    test "展示页面完成状态指示器", %{conn: conn, user: user, form: form} do
      ...
      assert page2_html =~ "phx-value-index=\"0\"" &&
               (page2_html =~ "complete" || page2_html =~ "active")
      ...
      assert page3_html =~ "phx-value-index=\"1\"" &&
               (page3_html =~ "complete" || page3_html =~ "active")
    end
