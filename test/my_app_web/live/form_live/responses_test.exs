defmodule MyAppWeb.FormLive.ResponsesTest do
  ...
  describe "表单响应列表页面" do
    setup %{user: user} do
      ...
      # 创建三个测试响应
      response1 =
        response_fixture(published_form.id, %{
          "#{text_item.id}" => "回答1",
  ...
  describe "表单响应详情页面" do
    ...
    test "查看响应详情", %{
      ...
      # 验证答案具有正确的值
      text_answer =
        Enum.find(response.answers, fn answer -> answer.form_item_id == text_item.id end)

      assert text_answer.value == %{"value" => "这是一个文本回答"}

      radio_answer =
        Enum.find(response.answers, fn answer -> answer.form_item_id == radio_item.id end)
    ...
    test "其他用户不能查看响应详情", %{conn: conn} do
      ...
      # 创建响应
      text_item =
        form_item_fixture(published_form, %{label: "问题", type: :text_input, required: true})
