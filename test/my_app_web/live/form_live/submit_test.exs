defmodule MyAppWeb.FormLive.SubmitTest do
  ...
  describe "表单提交页面" do
    ...
    test "表单字段正确显示", %{
      ...
      # 验证有必填字段，更贴近用户体验而非实现细节
      assert has_element?(view, "[required]")
    ...
    test "成功提交表单 - 创建表单响应", %{
      ...
      view
      |> render_hook("region_province_change", %{
        "field-id" => "#{region_item.id}",
        "value" => "广东省"
      })
      ...
      view
      |> render_hook("region_district_change", %{
        "field-id" => "#{region_item.id}",
        "value" => "南山区"
      })
