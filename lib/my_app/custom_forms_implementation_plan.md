...
1. **实现`textarea`控件**
   * [ ] 在`form_item_editor`中添加"文本区域"选项按钮
   * [ ] 实现`textarea_field/1`组件
   * [ ] 在表单预览和提交页面中支持文本区域渲染
   * [ ] 更新表单项编辑组件支持文本区域特定属性设置(如行数)

2. **实现`checkbox`复选框控件**
   * [ ] 在`form_item_editor`中添加"复选框"选项按钮
   * [ ] 实现`checkbox_field/1`组件
   * [ ] 添加复选框选项管理界面(类似单选按钮)
   * [ ] 在表单预览和提交页面中支持复选框渲染
   * [ ] 实现多选值的数据保存和验证

3. **实现`dropdown`下拉菜单控件**
   * [ ] 在`form_item_editor`中添加"下拉菜单"选项按钮
   * [ ] 实现`dropdown_field/1`组件
   * [ ] 添加下拉菜单选项管理界面
   * [ ] 在表单预览和提交页面中支持下拉菜单渲染
   * [ ] 实现下拉菜单值的数据保存和验证

4. **实现`rating`评分控件**
   * [ ] 在`form_item_editor`中添加"评分"选项按钮
   * [ ] 实现`rating_field/1`组件
   * [ ] 添加评分范围和样式设置界面
   * [ ] 在表单预览和提交页面中支持评分控件渲染
   * [ ] 实现评分值的数据保存和验证
...
**第一阶段优先级（2周）:**
1. 首先实现`textarea`和`dropdown`控件（基础且使用频率高）
2. 然后实现`checkbox`控件（常用多选功能）
3. 最后实现`rating`控件（评分功能）

**第二阶段优先级（3周）:**
1. 首先实现`date`和`phone`控件（常用个人信息字段）
2. 然后实现`name`控件（基础个人信息）
3. 再实现`region`地区选择控件（复杂但常用）
4. 最后实现`file_upload`控件（技术复杂度较高）
