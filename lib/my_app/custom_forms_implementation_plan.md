...
5. **添加`number`数字输入控件**
   * [ ] 在`FormItem`模型中添加`:number`类型
   * [ ] 在`form_item_editor`中添加"数字输入"选项按钮
   * [ ] 实现`number_field/1`组件
   * [ ] 添加数值范围和步长设置界面
   * [ ] 在表单预览和提交页面中支持数字输入控件渲染
   * [ ] 实现数值验证功能（范围检查、类型验证）

6. **添加`email`邮箱输入控件**
   * [ ] 在`FormItem`模型中添加`:email`类型
   * [ ] 在`form_item_editor`中添加"邮箱输入"选项按钮
   * [ ] 实现`email_field/1`组件
   * [ ] 添加邮箱格式验证
   * [ ] 在表单预览和提交页面中支持邮箱输入控件渲染
   * [ ] 实现邮箱格式提示功能

7. **添加`phone`电话号码控件**
   * [ ] 在`FormItem`模型中添加`:phone`类型
   * [ ] 在`form_item_editor`中添加"电话号码"选项按钮
   * [ ] 实现`phone_field/1`组件
   * [ ] 添加电话号码格式验证（国内手机号）
   * [ ] 实现区号输入与格式化显示
   * [ ] 在表单预览和提交页面中支持电话号码控件渲染
...
9. **添加`date`日期选择控件**
   * [ ] 在`FormItem`模型中添加`:date`类型
   * [ ] 在`form_item_editor`中添加"日期选择"选项按钮
   * [ ] 实现`date_field/1`组件
   * [ ] 添加日期格式和范围设置界面
   * [ ] 在表单预览和提交页面中支持日期选择控件渲染
   * [ ] 实现日期值的数据保存和验证
   * [ ] 提供日期选择器UI组件

10. **添加`time`时间选择控件**
    * [ ] 在`FormItem`模型中添加`:time`类型
    * [ ] 在`form_item_editor`中添加"时间选择"选项按钮
    * [ ] 实现`time_field/1`组件
    * [ ] 添加时间格式和范围设置界面
    * [ ] 在表单预览和提交页面中支持时间选择控件渲染
    * [ ] 实现时间值的数据保存和验证
    * [ ] 提供时间选择器UI组件

11. **添加`region`地区选择控件**
    * [ ] 在`FormItem`模型中添加`:region`类型
    * [ ] 在`form_item_editor`中添加"地区选择"选项按钮
    * [ ] 实现`region_field/1`组件
    * [ ] 添加省市区三级联动功能
    * [ ] 准备中国地区数据库（省/市/区县）
    * [ ] 在表单预览和提交页面中支持地区选择控件渲染
    * [ ] 实现地区数据的保存和验证

#### 阶段四：新增复杂交互控件

12. **添加`matrix`矩阵选择控件**
    * [ ] 在`FormItem`模型中添加`:matrix`类型
    * [ ] 在`form_item_editor`中添加"矩阵题"选项按钮
    * [ ] 实现`matrix_field/1`组件
    * [ ] 添加行/列选项管理界面
    * [ ] 支持单选和多选两种矩阵模式
    * [ ] 在表单预览和提交页面中支持矩阵控件渲染
    * [ ] 实现矩阵数据的保存和验证
...
13. **添加`image_choice`图片选择控件**
    * [ ] 在`FormItem`模型中添加`:image_choice`类型
    * [ ] 在`form_item_editor`中添加"图片选择"选项按钮
    * [ ] 实现`image_choice_field/1`组件
    * [ ] 添加选项图片上传和管理功能
    * [ ] 支持单选和多选两种图片选择模式
    * [ ] 在表单预览和提交页面中支持图片选择控件渲染
    * [ ] 实现图片选择数据的保存和验证

14. **添加`file_upload`文件上传控件**
    * [ ] 在`FormItem`模型中添加`:file_upload`类型
    * [ ] 在`form_item_editor`中添加"文件上传"选项按钮
    * [ ] 实现`file_upload_field/1`组件
    * [ ] 添加文件类型和大小限制设置
    * [ ] 支持多文件上传模式
    * [ ] 在表单预览和提交页面中支持文件上传控件渲染
    * [ ] 实现文件存储和访问功能

#### 阶段五：优化与完善

15. **控件分类与管理**
    * [ ] 将控件按功能类型分组(基础、个人信息、高级)
    * [ ] 优化控件选择界面，实现分类展示
    * [ ] 添加控件搜索功能
...
**第二阶段优先级（2周）:**
1. 首先实现`number`数字输入控件（基础控件，常用于数量、年龄等）
2. 然后实现`email`邮箱输入控件（基础个人信息字段）
3. 接着实现`phone`电话号码控件（常用联系信息）
...
6. **下一步工作计划** (待实施)
   * **测试代码重组** (计划中):
     ...
     - [ ] 创建 `test/my_app/forms_test.exs` 文件，集中测试所有控件的后端模型和业务逻辑 (进度: 0%)
     - [ ] 从现有测试中提取控件相关测试到新文件 (进度: 0%)
     - [ ] 为新添加的控件类型(特别是评分控件)编写专门的测试 (进度: 0%)
     - [ ] 保持控件测试与控件开发同步进行 (进度: 0%)
   * **进一步优化数据加载性能** (计划中):
   ...
   * **公开表单功能实现** (未开始):
     - [ ] 开发无需登录的公开表单显示功能 (进度: 0%)
     - [ ] 实现匿名用户表单提交功能 (进度: 0%)
     - [ ] 添加公开表单的访问控制和防滥用措施 (进度: 0%)
     - [ ] 为公开表单创建专用路由和控制器 (进度: 0%)
   * **前端用户体验改进** (未规划):
     - [ ] 优化表单编辑界面响应速度 (进度: 0%)
     - [ ] 改进表单项拖拽排序功能 (进度: 0%)
     - [ ] 添加表单预览功能 (进度: 0%)
