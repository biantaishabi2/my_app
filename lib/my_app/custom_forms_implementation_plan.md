...
**后端实现计划**
* [ ] 在 `FormItem` 模型中添加 `category` 字段，使用 Ecto.Enum 约束可选分类值：`:basic`（基础控件）、`:personal`（个人信息控件）、`:advanced`（高级交互控件）
* [ ] 创建迁移脚本 `add_category_to_form_items.exs`
* [ ] 设置控件默认分类：
  * 基础控件（`:basic`）：文本输入、文本区域、单选按钮、复选框、下拉菜单、数字输入
  * 个人信息控件（`:personal`）：邮箱、电话、日期、时间、地区
  * 高级控件（`:advanced`）：评分、矩阵选择、图片选择、文件上传
* [ ] 修改 `Forms.list_available_form_item_types/0` 函数返回按类别分组的控件类型

**前端实现计划**
* [ ] 重新设计表单控件选择界面，使用标签页或折叠面板组织不同类别的控件
* [ ] 为每个类别创建视觉图标，提升用户体验
* [ ] 创建新的 `form_item_type_selector/1` 组件，替换现有的简单列表
* [ ] 实现控件类别筛选功能
* [ ] 添加控件搜索功能，帮助用户快速找到需要的控件类型
* [ ] 优化移动端控件选择界面布局
