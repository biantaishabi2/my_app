defmodule MyApp.FormTemplates.FormTemplate do
  ...
  ## 返回值
    渲染后的HTML字符串
  """
  def render(%__MODULE__{structure: structure, decoration: decoration}, form_data)
      when is_map(form_data) do
    # --- 预计算需要编号的字段索引 ---
    # 识别需要编号的类型 (与渲染测试保持一致)
    ...
    # 合并并渲染所有元素
    rendered_html =
      render_combined_elements(filtered_structure, decoration_elements, form_data, field_indices)
  ...
  defp render_combined_elements(structure, decoration, form_data, field_indices) do
    ...
    decoration_elements =
      Enum.map(decoration, fn element ->
        element_id = Map.get(element, "id") || Map.get(element, :id)
    ...
    # 2. 创建ID到元素的映射，用于快速查找
    _element_map =
      (form_elements ++ decoration_elements)
    ...
    positioned_elements =
      ...
      |> Enum.reduce(positioned_elements, fn element, acc ->
        ...
        if position != nil && position["type"] in ["before", "after"] &&
             position["target_id"] != nil do
    ...
    main_elements =
      (form_elements ++ neutral_decorations)
      |> Enum.filter(fn element ->
    ...
    final_elements =
      Enum.reduce(main_elements, final_elements, fn element, acc ->
        element_id = Map.get(element, "element_id")
  ...
  # 渲染单个表单元素
  # 接收索引 Map
  defp render_element(element, form_data, field_indices) do
    # 提取元素属性
    elem_id = Map.get(element, "id") || Map.get(element, :id)
    type = Map.get(element, :type) || Map.get(element, "type")
    ...
    # --- 新增：获取当前元素的序号 ---
    # 如果是需要编号的字段，会得到序号，否则为 nil
    current_index = Map.get(field_indices, elem_id)
    # --- 结束新增 ---

    # 根据元素类型渲染不同的HTML
    case type do
      "text" ->
        render_text_input(name, label, value, current_index)

      "number" ->
        render_number_input(name, label, value, current_index)
      ...
      # 也传递给默认渲染器
      _ ->
        render_default_input(type, name, label, value, current_index)
    end
  end
  ...
  def evaluate_condition(condition, form_data) do
    cond do
      # 简单条件（如：字段 == 值）
      is_map_key(condition, :operator) && is_map_key(condition, :left) &&
          is_map_key(condition, :right) ->
  ...
  defp evaluate_simple_condition(%{operator: operator, left: left, right: right}, form_data) do
    ...
    case operator do
      ...
      ">" ->
        try do
          {left_num, _} =
            if is_binary(left_value), do: Integer.parse(left_value), else: {left_value, nil}

          {right_num, _} =
            if is_binary(right_value), do: Integer.parse(right_value), else: {right_value, nil}
