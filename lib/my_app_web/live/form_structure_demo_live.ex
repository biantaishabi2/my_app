...
defmodule MyAppWeb.FormStructureDemoLive do
  ...
  # 定义 JSON 文件路径
  @template_json_path "priv/static/templates/form_structure_demo.json" # 使用新的 JSON 文件
  ...
  @impl true
  def handle_event("move_item", %{"old_index" => old_index_str, "new_index" => new_index_str}, socket) do
  ...
  def render(assigns) do
    ...
    <div class="container mx-auto p-6">
      ...
      <div id="structure-list" phx-hook="Sortable" class="space-y-2 border p-4 rounded bg-white shadow">
        ...
        <% else %>
          <%= for element <- @structure do %>
            ...
            %>
            <div id={"item-#{elem_id}"} data-id={elem_id} class="flex items-center p-3 border rounded bg-gray-100 hover:bg-gray-200 transition duration-150 ease-in-out" draggable="true">
  ...
  def handle_event("update_structure_order", %{"ordered_ids" => ordered_ids}, socket) do
    IO.puts("Received new order: #{inspect(ordered_ids)}") # 调试输出
    ...
    new_structure =
      ordered_ids
      |> Enum.filter(&(&1 != nil)) # 过滤掉 nil
      |> Enum.map(&Map.get(structure_map, &1)) # 通过 ID 从映射中查找元素
      |> Enum.filter(&(&1 != nil)) # 再次过滤，确保找到的元素存在
    ...
    if length(new_structure) == length(current_structure) do
      ...
      IO.puts("Error: Reordered structure length mismatch. Check data-id attributes and received IDs.")
      # 可以选择不更新，或者添加错误提示
    ...
    case File.read(@template_json_path) do
      {:ok, content} ->
        case Jason.decode(content) do
          ...
          #   {:ok, structure}
          {:ok, structure_list} when is_list(structure_list) -> # 直接解码为列表
