defmodule MyAppWeb.ChatLive.Index do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     assign(socket,
       ...
       # Store the full conversation struct
  ...
  def handle_params(%{"id" => id}, _uri, socket) do
    ...
    try do
      ...
      {:noreply,
       assign(socket, current_conversation: conversation, messages: conversation.messages)}
    rescue
      Ecto.NoResultsError ->
        {:noreply,
         socket
      ...
      # Handle other potential errors like invalid ID format
      _ ->
        {:noreply,
         socket
  ...
  def handle_event("send_message", %{"message" => content}, socket) when content != "" do
    ...
    if is_nil(current_conversation_maybe_temp) do
      ...
      persisted_conversation =
        case current_conversation_maybe_temp.id do
          :new ->
            ...
            new_conv_attrs = %{title: String.slice(content, 0, 30)}
      ...
      if persisted_conversation do
        ...
        {:ok, _user_msg} =
          Chat.create_message(persisted_conversation, %{role: "user", content: content})

        # 2. Simulate and create AI response
        ai_response_content = generate_ai_response(content)

        {:ok, _ai_msg} =
          Chat.create_message(persisted_conversation, %{
            role: "assistant",
            content: ai_response_content
          })

        # 3. Reload conversation WITH messages *after* inserts
        reloaded_conv_after_inserts =
          Chat.get_conversation_with_messages!(current_user, persisted_conversation.id)
        ...
        # Pass empty map if only timestamp update
        {:ok, final_updated_conversation} =
          Chat.update_conversation(reloaded_conv_after_inserts, %{})

        # 5. Reload AGAIN to get final state
        final_reloaded_conversation =
          Chat.get_conversation_with_messages!(current_user, final_updated_conversation.id)
  ...
  def handle_event("new_conversation", _params, socket) do
    ...
    {:noreply,
     assign(socket,
       # Assign the list including the new temp one
  ...
  def handle_event("select_conversation", %{"id" => id_str}, socket) do
    ...
    # 如果正在编辑或删除，忽略选择事件
    if socket.assigns.editing_conversation_id != nil ||
         socket.assigns.deleting_conversation_id != nil do
      {:noreply, socket}
    else
      try do
  ...
  def handle_event(
        ...
        %{"conversation_id" => id_str, "title" => title},
        socket
      ) do
    current_user = socket.assigns.current_user
    id = String.to_integer(id_str)
  ...
  def handle_event("save_conversation_name", params, socket) do
    ...
    if editing_id && Map.has_key?(params, "title") do
      ...
      handle_event(
        "save_conversation_name",
        %{"conversation_id" => to_string(editing_id), "title" => params["title"]},
        socket
      )
    else
      # 其他情况，取消编辑状态
  ...
  defp generate_ai_response(user_input) do
    ...
    cond do
      String.contains?(user_input, "你好") ->
        "你好！很高兴见到你！有什么我可以帮助你的？"

      String.contains?(user_input, "天气") ->
        "我没有实时的天气数据，但你可以查看本地气象网站获取最新天气信息。"

      String.contains?(user_input, "谢谢") ->
        "不客气！如果有其他问题，随时可以问我。"

      true ->
        "我理解你的问题是关于\"#{String.slice(user_input, 0, 30)}#{if String.length(user_input) > 30, do: "...", else: ""}\"。作为一个AI助手，我可以提供相关信息和帮助解决问题。请告诉我更多详细信息，这样我能更好地帮助你。"
    end
  end

  # This function might not be needed if using Ecto timestamps directly in the template
  defp format_time(datetime)
       when is_struct(datetime, NaiveDateTime) or is_struct(datetime, DateTime) do
