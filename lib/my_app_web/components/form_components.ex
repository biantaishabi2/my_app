defmodule MyAppWeb.FormComponents do
  ...
  def condition_editor(assigns) do
    ...
    <!-- 条件操作符选择 -->
              ...
              >
                ...
                <option value="greater_than" selected={@condition.operator == "greater_than"}>
                  大于
                </option>
                <option
                  value="greater_than_or_equal"
                  selected={@condition.operator == "greater_than_or_equal"}
                >
                  大于等于
                </option>
                ...
                <option
                  ...
                  selected={@condition.operator == "less_than_or_equal"}
                >
                  小于等于
                </option>
              </select>
  ...
  def text_input_field(assigns) do
    ...
    <div class="form-field form-item mb-4">
      <label
        for={@field.id}
        class={"block text-sm font-medium mb-1 #{if @field.required, do: "required", else: ""}"}
      >
        {@field.label}
      ...
      <div class="form-items-container space-y-4 mb-6">
        ...
        <% else %>
          <%= for {item, index} <- Enum.with_index(@items) do %>
            <div
              class="form-item-wrapper bg-white p-4 border rounded-md shadow-sm"
              id={"item-#{item.id}"}
              data-order={index + 1}
            >
              <div class="flex justify-between items-center mb-2">
                <div class="font-medium">
                  {index + 1}. {item.label} ({display_item_type(item.type)})
                </div>
                <div class="flex space-x-2">
                  <button
                    type="button"
                    phx-click={@on_edit_item.(item.id)}
                  ...
                  <button
                    type="button"
                    phx-click={@on_delete_item.(item.id)}
          ...
          <div class="pt-4 border-t border-gray-200">
            <div class="mb-4">
              ...
              >
                <%= for val <- 3..10 do %>
                  <option
                    value={val}
                    selected={@item.max_rating == val || (@item.max_rating == nil && val == 5)}
                  >
                    {val} 星
                  </option>
                <% end %>
            ...
            <div class="rating-preview p-3 bg-gray-50 rounded-md">
              ...
              <div class="flex items-center">
                <%= for i <- 1..5 do %>
                  <span class={"text-2xl #{if i <= 3, do: "text-yellow-500", else: "text-gray-300"}"}>
                    ★
                  </span>
                <% end %>
          ...
          <div class="pt-4 border-t border-gray-200">
            ...
            <div class="mt-4">
              ...
              >
                <option
                  value="yyyy-MM-dd"
                  selected={@item.date_format == "yyyy-MM-dd" || is_nil(@item.date_format)}
                >
                  年-月-日 (yyyy-MM-dd)
                </option>
                <option value="dd/MM/yyyy" selected={@item.date_format == "dd/MM/yyyy"}>
                  日/月/年 (dd/MM/yyyy)
                </option>
                <option value="MM/dd/yyyy" selected={@item.date_format == "MM/dd/yyyy"}>
                  月/日/年 (MM/dd/yyyy)
                </option>
              </select>
            </div>
          ...
          <div class="pt-4 border-t border-gray-200">
            ...
            <div class="mt-4">
              ...
              <select
                ...
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
                <option value="24h" selected={@item.time_format == "24h" || is_nil(@item.time_format)}>
                  24小时制 (HH:mm)
                </option>
          ...
          <div class="pt-4 border-t border-gray-200">
            ...
            <div>
              ...
              >
                ...
                <option value="3" selected={@item.region_level == 3 || is_nil(@item.region_level)}>
                  省市区三级
                </option>
              </select>
            </div>
            <div class="mt-4">
              ...
              >
                ...
                <%= for province <- MyApp.Regions.get_provinces() do %>
                  <option value={province.name} selected={@item.default_province == province.name}>
                    {province.name}
                  </option>
                <% end %>
              </select>
            ...
            <div class="mt-4 p-3 bg-gray-50 rounded-md">
              ...
              <div class="flex gap-2">
                <select disabled class="flex-1 px-3 py-2 border border-gray-300 rounded-md bg-white">
              ...
              <div class="flex space-x-4">
                ...
                <label class="inline-flex items-center">
                  <input
                    ...
                    value="bottom"
                    checked={
                      @item.image_caption_position == :bottom || @item.image_caption_position == nil
                    }
                    class="h-4 w-4 text-indigo-600 focus:ring-indigo-500"
                  />
    ...
    <!-- 文件扩展名设置 -->
            <div class="mb-4">
              ...
              <input
                ...
                value={
                  if is_list(@item.allowed_extensions),
                    do: Enum.join(@item.allowed_extensions, ", "),
                    else: ""
                }
              ...
              <div class="border-2 border-dashed border-gray-300 rounded-md p-6 text-center">
                ...
                <div class="mt-1 text-xs text-gray-500">
                  <%= if @item.allowed_extensions && @item.allowed_extensions != [] do %>
                    支持格式: {if is_list(@item.allowed_extensions),
                      do: Enum.join(@item.allowed_extensions, ", "),
                      else: @item.allowed_extensions}
                  <% else %>
                    支持所有文件格式
                  <% end %>
                </div>
              ...
              <div class="p-1 border-t border-gray-300 bg-white text-center">
                <%= if @field.selection_type == :multiple do %>
                  <input
                    ...
                    value={option.value}
                    checked={
                      is_list(Map.get(@form_state, @field.id)) &&
                        option.value in Map.get(@form_state, @field.id, [])
                    }
                    disabled={@disabled}
                    class="h-4 w-4 text-indigo-600 focus:ring-indigo-500"
                    phx-debounce="blur"
  ...
  def file_upload_field(assigns) do
    ...
    <div class="form-field form-item mb-6 file-upload-container">
      <label
        for={@field.id}
        class={"block text-sm font-medium mb-2 #{if @field.required, do: "required", else: ""}"}
      >
        {@field.label}
        <%= if @field.required do %>
          <span class="form-item-required text-red-500">*</span>
      ...
      <div class="file-upload-dropzone" id={"dropzone-#{@field.id}"} phx-hook="FileUploadDropzone">
        ...
        <%= if @form_state && @form_state[@field.id] && length(@form_state[@field.id]) > 0 do %>
          <div class="file-list-container">
            ...
            <ul class="file-list">
              <%= for {file, _index} <- Enum.with_index(@form_state[@field.id]) do %>
                <li class="file-item">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
