    ...
    <div class="space-y-2">
      ...
      <div class={"grid grid-cols-#{@region_level} gap-2"}>
        ...
        <div class="relative region-select-container" id={"region-province-container-#{@field_id}"}>
          ...
          <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-500" style="height: 40px;">
      ...
      <%= if Map.get(@errors, @field_id) do %>
        <div id={"error_#{@field_id}"} class="text-red-500 text-sm mt-1 error-message" role="alert"><%= @errors[@field_id] %></div>
      <% end %>
    </div>
    """
  end
